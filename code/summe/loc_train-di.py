...
def train_loc(
        ...
        for epoch in range(set_epochs):
            ...
            for video_features, word_ids, char_ids, _, _, s_labels, e_labels, hl_labels, num_units, _ in train_loader:
                ...
                if step_counter % args.eval_period == 0: # 10마다
                    val_fscore, _, _, _, _, _ = sum_eval(model_sum, val_loader_sum, args.nms_thresh, args.device)
                    val_fscore_tech, _, _, _, _, _ = sum_eval(model_sum_tech, val_loader_sum, args.nms_thresh, args.device)
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                for _, video_seq, gtscore, change_points, n_frames, nfps, picks, user_summary, sum_ratio in train_loader_sum:
                    ...
                    score_loss = sum_score_loss(score_pred, summary)
    ...
    return model_sum, model_sum_tech, optimizer1, optimizer2, model_loc, model_loc_tech, max_val_fscore
...
def sum_eval(model, val_loader, nms_thresh, device):
    ...
    with torch.no_grad():
        ...
        for test_key, seq, gtscore, cps, n_frames, nfps, picks, user_summary, sum_ratio in val_loader:
            ...
            for i in range(len(user_summary)):
                ...
                score_pred, pred_boundbox, t_loss, t_score_loss, t_reg_loss, t_cen_loss= model.predict(seq_torch, summary, reg_label, cen_label)
                loss += t_loss
    ...
    return stats.fscore, loss, score_loss, reg_loss, cen_loss, logits_dict
...
def sum_score_loss(score_pred, target):
    ...
    loss = focal_loss(score_pred, target)
